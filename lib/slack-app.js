...
module.exports.start = (controller, config, plugins) => {
  ...
  server.start(controller, config, (err) => {
    if (err) {
      return;
    }
    ...
    controller.storage.teams.all((err, teams) => startTeamsBots(err, controller, config, teams))
...
function startTeamsBots (err, controller, config, teams) {
  if (err) {
